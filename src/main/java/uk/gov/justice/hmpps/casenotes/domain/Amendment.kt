...
interface AmendmentRepository : JpaRepository<Amendment, UUID> {
  ...
  @Query(
    """
      ...
      using case_note c join case_note_sub_type st on st.id = c.sub_type_id 
      where a.case_note_id = c.id
      and c.person_identifier = :personIdentifier and st.sync_to_nomis = true 
      """,
    nativeQuery = true,
  )
